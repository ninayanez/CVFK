...
export function makeBox (pos, name) {
...
// module returns specific paper object class

// paper project acts as global workspace
    ...
    _.each(sketch.boxes, (b) => { 
      ...
      if (inside||sect) {
        if (e.shiftKey) { // DESELECT!
          ...
          itemDeselect(b) 
          p.project.activeLayer.addChildren([b])
        } else {
          if (sketch.selected.isChild(b)) return
          itemSelect(b) 
          sketch.selected.addChildren([b])
        }
      } else if (!e.shiftKey) {
        itemDeselect(b) 
        p.project.activeLayer.addChildren([b])
    ...
    _.each(sketch.cords, (c) => {
      if (selection.intersects(c.item)) {
        if (e.shiftKey) { // DESELECT!
          ...
          itemDeselect(c.item) 
          p.project.activeLayer.addChildren([c.item])
        } else {
          if (sketch.selected.isChild(c.item)) return
          itemSelect(c.item) 
...
window.addEventListener('keydown', (e) => {
  if (e.keyCode===8) { /// mmmmmmmmm
    let del = _.clone(sketch.selected.children)
    console.log('boxes',sketch.boxes)
    console.log('cords',sketch.cords)    
    _.each(del, (c) => { 
    ...
    console.log('boxes',sketch.boxes)
    console.log('cords',sketch.cords)
    // sketch.selected.removeChildren()
    // _.each(del, (d) => {
    //   console.log(p.project.activeLayer.children[d])
    // })
    p.view.draw()
  }
}, false)
