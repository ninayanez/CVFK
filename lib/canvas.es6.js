...
let node = class { // patch node!

}

// define a box class!
let box = class extends p.Group {
    ...
    _.each(p.project.activeLayer.children, (b) => { 
      ...
      if (inside||sect) {
        if (e.shiftKey) { // DESELECT!
          if (sketch.selected.isChild(b)) itemDeselect(b) 
        } else {
          if (!sketch.selected.isChild(b)) itemSelect(b) 
        }
      } else if (!e.shiftKey) itemDeselect(b) 
    })
    p.view.draw()
  }
}
...
function itemDeselect (item) {
  p.project.activeLayer.addChildren([item])
...
function itemSelect (item) {
  sketch.selected.addChildren[item]
  if (item instanceof p.Group) {
