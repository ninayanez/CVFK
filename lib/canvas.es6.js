...
function moveBox (item) {
  ...
  if (p.project.selectedItems.length > 0) { // move selected as well
    _.each(p.project.selectedItems, (i) => {

    })
  }
  return function mouseMoveListener (e) { 
    t.position = [ev.clientX-off.x, ev.clientY-off.y]
    if (inlet) {
      let patch = sketch.cords[inlet]
      let point = t.children[3]
      patch.item.segments = [
...
function dragBox (e) {
  ...
  let off = getOffset(e.point.x,e.point.y)
