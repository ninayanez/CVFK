...
function canvasMouseDown (e) { // select
  ...
  window.onmousemove = function (ev) {
    ...
    _.each(layerDefault.children, (b) => { 
      ...
      if (inside||sect) selected.push(b.name)
    })
    _.each(selected, (cid) => {
      if (layerSelected.children[cid]) selectItem(getItem(cid), false)
      if (!layerSelected.children[cid]) selectItem(getItem(cid), true)
      if (e.shiftKey) {  }
    })
    p.view.draw()
  }
